...
<html>
    ...
    <body>
        <h1 style="text-align: center;color:black;">ENTER THE CUSTOMER ID TO GENERATE OTP</h1>
        ...
        border-radius: 10px;" >
            <form method ="post" id="register" action="" onclick="aforgotpassword.php">
                ...
                <br/>
                <input type ="number" id ="adminid" name="cid" placeholder="Enter the Customer Id " ><br>
		...
		if(isset($_POST['sendcode']))
		{
			...
			{	
				...
				$sql = "SELECT  * FROM customer_pro where c_id='$_POST[cid]'";
	...
	if($cid == $_POST['cid'] && !empty ($_POST['cid']))
	{
...
//$mail->SMTPDebug = 3;                               // Enable verbose debug output
	...
	$mail->Subject = 'Verificatition Email for Car Showroom Customer';
	$randomcode=mt_rand(0,999999);
	$_SESSION['cid']=$cid;
	$_SESSION['random']=$randomcode;
	...
	if(!$mail->send()) {
	    ...
	    echo 'Mailer Error: ' . $mail->ErrorInfo;
			...
			if($code == $_SESSION['random'])
			{
				...
				header('location:cresetpassword.php');
			}
			else
			{
